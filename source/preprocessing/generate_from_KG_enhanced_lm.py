...
def main():
    ...
    parser.add_argument("--max_single_rationale_length", default=15, type=int, required=False,
    ...
    generator = LMTextGenerator(args.lm, args.model_name_or_path, device=device)

    prefixes = {
        "definition": ["What is the definition of \"{}\"?", "The definition of \"{}\" is"],
        "purpose": ["What is the purpose of \"{}\"?", "The purpose of \"{}\" is"],
        "relation": ["What is the relationship between \"{}\" and \"{}\"?", "The relationship between \"{}\" and \"{}\" is that"],
    ...
    for key in ["relation"]: #, "difference"]:
        ...
        generated_rationales = [
            ...
            (prefixes[key][0].format(query_pair[i][0], query_pair[i][1]),
             ' '.join(
                 (f"{prefixes[key][1].format(query_pair[i][0], query_pair[i][1])}", clar_q))) # (f"{prefixes[key][1].format(query_pair[i][0], query_pair[i][1])} that {query_pair[i][0]}", clar_q)))
            for i, clar_qs in generated_rationales.items()
            for clar_q in clar_qs
        ]
