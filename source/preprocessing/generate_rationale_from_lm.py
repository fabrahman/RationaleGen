...
def main():
    ...
    generator = LMTextGenerator(args.lm, args.lm, device=device)
    ...
    with open(args.dataset, "r") as f_in:
        with open(args.out_file, "w") as f_out:
            ...
            for line in tqdm.tqdm(f_in, total=num_lines):
                ...
                pair_queries = [pair for pair in pair_queries if pair[0] != pair[1]]
                pair_queries = sorted(
                    pair_queries,
                    key=lambda pair: similarity(nlp, *pair),
                    reverse=True
                )[:3]
                pair_context = update + " " + hypothesis
...
def generate_pair_clarifications(generator: LMTextGenerator,
    ...
    context = " ".join((premise, aux_context)) # context = premise
    ...
    for key in ["relation"]: #, "difference"]:
        ...
        generated_rationales = [
            ...
            (prefixes[key][0].format(query_pair[i][0], query_pair[i][1]),
             ' '.join(
                 (f"{prefixes[key][1].format(query_pair[i][0], query_pair[i][1])} that", clar_q))) # (f"{prefixes[key][1].format(query_pair[i][0], query_pair[i][1])} that {query_pair[i][0]}", clar_q)))
            for i, clar_qs in generated_rationales.items()
            for clar_q in clar_qs
        ]
